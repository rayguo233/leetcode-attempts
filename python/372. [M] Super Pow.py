...
class Solution:
    def superPow(self, a: int, b: List[int]) -> int:
        ...
        product_set = set()
        curr = a % 1337
        while curr not in product_set:
            products.append(curr)
            product_set.add(curr)
            curr = (curr * a) % 1337
        circ_len = len(products)


        # print(70 in product_set, products)
        return products[self.super_mod(circ_len, b) - 1]

    def super_mod(self, mod: int, b: List[int]) -> int:
        CONST = 4
        short_b = b if len(b) <= CONST else b[-CONST:]
        real_short_b = 0
        for i, digit in enumerate(reversed(short_b)):
            real_short_b += digit * (10 ** i)
        if len(b) <= CONST:
            # print('-------', mod, b, real_short_b % mod)
            return real_short_b % mod
        upper_res = self.super_mod(mod, b[:-CONST])
        upper_res = [int(digit) for digit in str(upper_res)] if upper_res else [0]
        res = (self.super_mod(mod, upper_res) * (10 ** CONST) + (real_short_b % mod)) % mod
        # print('-------', self.super_mod(mod, upper_res) * (10 ** CONST), real_short_b, mod, b, res)
        return res
...
if __name__ == '__main__':
    ...
    # print(sol.superPow(1, [4,3,3,8,5,2]))
    # print(sol.superPow(2147483647, [2,0,0]))
    # print(sol.super_mod(3, [1,0,0,0,0]), 10000 % 3)
    # print(sol.super_mod(3, [1] + [0] * 10))
